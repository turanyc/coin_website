                          ...
                          <button
                            onClick={async (e) => {
                              ...
                              try {
                                ...
                                if (user) {
                                  // Kullanıcı giriş yapmışsa API'ye direkt ekle
                                  ...
                                  const response = await fetch('/api/portfolio', {
                                    ...
                                    body: JSON.stringify({
                                      ...
                                      coin_name: coin.name,
                                      amount: 1,
                                      purchase_price: coin.current_price || null,
                                      purchase_date: null,
                                      notes: null,
                                    }),
                                  });
                                  ...
                                  if (response.ok) {
                                    // Başarılı - portföy sayfasına yönlendir
                                    // Kısa bir gecikme ile yönlendir (API işleminin tamamlanması için)
                                    setTimeout(() => {
                                      router.push('/portfolio');
                                    }, 100);
                                  } else {
                                    const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
                                    alert(`Coin eklenirken bir hata oluştu: ${errorData.error || 'Bilinmeyen hata'}`);
                                  }
                                } else {
                                  // Cookie kontrolü
                                  const { hasCookieConsent, addCoinToPortfolioCookie } = await import('../lib/cookieUtils');
                                  if (hasCookieConsent()) {
                                    addCoinToPortfolioCookie({
                                      coinId: coin.id,
                                      coinSymbol: coin.symbol,
                                      coinName: coin.name,
                                      amount: 1,
                                      purchasePrice: coin.current_price || undefined,
                                      purchaseDate: undefined,
                                      notes: undefined,
                                    });
                                    // Başarılı - portföy sayfasına yönlendir
                                    // Kısa bir gecikme ile yönlendir (localStorage işleminin tamamlanması için)
                                    setTimeout(() => {
                                      router.push('/portfolio');
                                    }, 100);
                                  } else {
                                    alert('Lütfen önce çerezleri kabul edin veya giriş yapın.');
                                  }
                                }
                              } catch (error) {
                            ...
                            className="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 bg-gray-100 hover:bg-[#2563EB] hover:text-white text-gray-400"
                            title="Portföye ekle"
                          >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
